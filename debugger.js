...
(function (window) {
  ...
  if (window.dataLayer_debugger === undefined) {
    ...
    dldb.validate = function (validKeys, testObj, type) {
      ...
      if (check.length !== validKeys.length){
        ...
        //try {
          //throw err;
        //} catch (err) {
          console.log("Invalid " + type + " object pushed to dataLayer. Missing: " + validKeys.join(", ") + " properties.");
        //}
        return true;
      }
      return false;
        ...
        //  try {
            ...
            for (var i = 0; i< ks.length; i++) {
              ...
              if (key.match(/^transaction(Id|Total|Affiliation|Shipping|Tax|Products)$/)){
                ...
                if ( window.dataLayer_debugger.validate("transaction", ['transactionTotal','transactionId'], obj, "transaction") ) {
                  break;
                }
              }
